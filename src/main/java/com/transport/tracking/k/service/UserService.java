...
//        User user = this.userRepository.findByXloginAndXpswdAndXact(userVO.getXlogin(), userVO.getXpswd(),2);
    ...
    public UserVO opsLogin(@NotNull UserVO userVO, HttpServletResponse response) {
        User user = this.userRepository.findByXloginAndXpswd(userVO.getXlogin(), userVO.getXpswd());
        ...
        // Check account active as before
        if (user.getXact() != 2) {
            throw new RuntimeException("The User is currently inactive. Unable to login");
        }

        // NEW: only allow OPS login when XOPSDASH == 2
        // Replace getXopsdash() with the actual getter name if different (e.g. getXopsdash0())
        Integer xopsdashVal = user.getXopsdash();
        if (xopsdashVal == null || xopsdashVal.intValue() != 2) {
            throw new RuntimeException("User doesn't have access");
        }

        // Build successful response (same as existing login flow)
        userVO = new UserVO();
        userVO.setXusrname(user.getXusrname());
        userVO.setXlogin(user.getXlogin());
        userVO.setXact(user.getXact());
        ...
        List<String> permissions = new ArrayList();
        permissions.add("Admin"); // keep existing behavior or derive permissions from DB
